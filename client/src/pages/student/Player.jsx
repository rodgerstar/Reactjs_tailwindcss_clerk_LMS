...
const Player = () => {
    const {
        enrolledCourses, calculateChapterTime, backendUrl, getToken, userData,
        fetchUserEnrolledCourses
    } = useContext(AppContext);
    const {courseId} = useParams();
    const [courseData, setCourseData] = useState(null);
    // Changed initial state to an object with all sections set to false
    ...
    // Modified toggleSection to simply toggle the clicked section independently
    const toggleSection = (index) => {
        setOpenSection((prev) => ({
            ...prev,
            [index]: !prev[index] || false // Toggle between true/false, default to false if undefined
        }));
    };
    ...
    const handleRate = async (rating) => {
        try {
            ...
            if (data.success) {
                toast.success(data.message);
                setInitialRating(rating);  // <-- Update UI immediately
                fetchUserEnrolledCourses(); // Refresh data from the backend
            } else {
                toast.error(data.message);
            }
        } catch (error) {
                        ...
                        <div>
                            ...
                            <div className='flex justify-between items-center mt-1'>
                                ...
                                <button onClick={() => markLectureAsCompleted(playerData.lectureId)}
                                        className='text-blue-600'>{progressData && progressData.lectureCompleted.includes(playerData.lectureId) ? 'Completed' : 'Mark Completed'}</button>
                            </div>
                        </div>
                    ) : (
