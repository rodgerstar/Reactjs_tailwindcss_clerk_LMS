...
const AddCourse = () => {
        ...
        setChapters(
            chapters.map((chapter) => {
                if (chapter.chapterId === currentChapterId) {
                    const newLecture = {
                        ...lectureDetails,
                        lectureOrder: chapter.chapterContent.length > 0 ? chapter.chapterContent.slice(-1)[0].lectureOrder
                            + 1 : 1,
                        lectureId: uniqid()
                    };
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const token = await getToken(); // Call the function with ()
            const {data} = await axios.post(backendUrl + 'api/educator/add-course', formData, { // Remove {formData} nesting
                headers: {
                        ...
                        showPopup && (
                            ...
                            bg-gray-800 bg-opacity-50'>
                                ...
                                max-w-80'>
                                    ...
                                    px-4 py-2 rounded' onClick={addLecture}>Add
                                    </button>
